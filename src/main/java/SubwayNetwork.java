...
public class SubwayNetwork {
    ...
    public void findShortestPath (SubwayStation.Station origin, SubwayStation.Station destination) throws IOException {
        final Converter converter = new Converter();
        stations = converter.getStations();
//        1. Mark all nodes unvisited. Create a set of all the unvisited nodes called the unvisited set.
        SubwayStation.Station initialNode = origin;
        List<SubwayStation.Station> unvisitedSet = stations.get(origin.getProperties().getObjectid()).getConnections(converter, origin);

//        2. Assign to every node a tentative distance value: set it to zero for our initial node and to infinity for all other nodes. Set the initial node as current.
        initialNode.getProperties().setDistance(0);
        initialNode.getProperties().setCurrent(true);
...
//        Compare the newly calculated tentative distance to the current assigned value and assign the smaller one. For example, if the current node A is marked with a distance of 6, and the edge connecting it with a neighbour B has length 2, then the distance to B through A will be 6 + 2 = 8. If B was previously marked with a distance greater than 8 then change it to 8. Otherwise, the current value will be kept.
        for (SubwayStation.Station connectingStation : unvisitedSet) {
            connectingStation.getProperties().setDistance(1); 
        }
